...
public class VehiculoController implements Initializable {
    ...
    private void verDetalles(ActionEvent event) {
        ...
        @FXML
    private void buscarPorId(KeyEvent event) {
        if (event.getCode() == KeyCode.ENTER) {
            String texto = tfBuscarId.getText().trim();
            ...
            try {
                ...
                Vehiculo vehiculo = gestor.obtenerPorId(id);
                
                // Mostrar solo el vehículo encontrado
                List<Vehiculo> vehiculoEncontrado = new ArrayList<>();
                vehiculoEncontrado.add(vehiculo);
                vehiculosObservable.setAll(vehiculoEncontrado);
                ...
                mostrarAlerta("Vehículo no encontrado", e.getMessage());
                tfBuscarId.clear();
            }
        }
    }
    ...
    private void limpiarBusqueda(ActionEvent event) {
        ...
        actualizarTabla();
        modoFiltrado = false;
        // Restablecer el filtro del ComboBox
        cbFiltro.setValue("Todos");
    }
    @FXML
